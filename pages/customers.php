...
// Total count
$total_result = $conn->query("SELECT COUNT(*) as cnt FROM customers");
...
$customers = $conn->query("
    SELECT c.*, COUNT(DISTINCT d.id) as do_cnt
    FROM customers c
    LEFT JOIN delivery_orders d ON d.customer_id = c.id
    GROUP BY c.id
    ORDER BY c.name
...
<div class="card">
    <div class="tbl-wrap">
        <table>
            ...
            <tbody>
                <?php $i = $offset + 1; while ($row = $customers->fetch_assoc()): ?>
                <tr>
                    <td><?= $i++ ?></td>
