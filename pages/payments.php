...
$total_result  = $conn->query("SELECT COUNT(*) as cnt FROM payments p $where");
    ...
    <div class="tbl-wrap">
        <table>
            ...
            ?>
                    ...
                    <td><span
                            class="badge badge-<?= $row['payer_type'] === 'customer' ? 'send' : ($row['payer_type'] === 'supplier' ? 'pct' : 'primary') ?>"><?= ucfirst($row['payer_type']) ?></span>
                    </td>
                    <td class="td-num">
                        <?= number_format(($row['payment_type']==='return' ? -$row['amount'] : $row['amount']), 2) ?>
                    </td>
                    <td><span
                            class="badge badge-<?= $row['payment_type'] === 'payment' ? 'payment' : 'return' ?>"><?= $row['payment_type'] ?></span>
                    </td>
        ...
        <form method="POST">
            ...
            <div class="modal-body">
                <div class="form-group"
                    style="margin-bottom: 15px; border: 2px dashed #007bff; padding: 20px; text-align: center; border-radius: 8px; background: #f8f9fa;">
                    <label for="receipt_upload"
                        style="cursor:pointer; font-weight:bold; color:#007bff; display:block; margin:0;">
                        <i class="fas fa-upload" style="margin-right:8px; font-size:1.2em;"></i> Upload Receipt to
                        Auto-fill
                    </label>
                    <input type="file" id="receipt_upload" accept="image/*" style="display:none;"
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="receipt_number" class="form-control"
                            placeholder="e.g., RCP-001, Invoice #123">
                    </div>
                    <div class="form-group">
                  ...
                  $label = date('d/m/Y', strtotime($ct['contract_date'])) . ' | ' .
                           ...
                           (float)$ct['rate'];
                ?>
                            <option value="<?= $ct['id'] ?>">Contract #<?= $ct['id'] ?> -
...
async function processReceipt(input) {
    ...
    statusDiv.innerHTML =
        '<span style="color:#f39c12"><i class="fas fa-spinner fa-spin"></i> Processing receipt, please wait...</span>';
    ...
    try {
        ...
        try {
            ...
            throw new Error(
                `OCR upload failed with status ${response.status}. Server response was not valid JSON.`);
        }
        ...
        const priorityHints = [
            ['grand total', 'net total', 'net payable', 'net amount', 'total amount', 'total due',
                'balance due', 'amount due', 'payable amount'
            ],
            ...
            for (let line of cleanLines) {
                ...
                if (hintGroup.some(hint => lower.includes(hint))) {
                    ...
                    if (nums.length > 0) {
                        ...
                        if (candidate > maxAmount && candidate >= 10) {
                            maxAmount = candidate;
                            amountLine = line;
                            found = true;
                        }
        ...
        const dateRegexes = [{
                regex: /(?:date|dated|dt\.?|on)\s*[:\-]?\s*(\d{1,2})[\/\-\.](\d{1,2})[\/\-\.](\d{2,4})/i,
                format: 'DMY'
            },
            {
                regex: /(?:date|dated|dt\.?|on)\s*[:\-]?\s*(\d{4})[\/\-\.](\d{1,2})[\/\-\.](\d{1,2})/i,
                format: 'YMD'
            },
            {
                regex: /(?:date|dated|dt\.?|on)\s*[:\-]?\s*(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+(\d{1,2})(?:st|nd|rd|th)?,?\s+(\d{4})/i,
                format: 'MDY_STR'
            },
            {
                regex: /(?:date|dated|dt\.?|on)\s*[:\-]?\s*(\d{1,2})(?:st|nd|rd|th)?\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s*,?\s*(\d{4})/i,
                format: 'DMY_STR'
            },
            {
            ...
            {
                regex: /\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s+(\d{1,2})(?:st|nd|rd|th)?,?\s+(\d{4})\b/i,
                format: 'MDY_STR'
            },
            {
                regex: /\b(\d{1,2})(?:st|nd|rd|th)?\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\s*,?\s*(\d{4})\b/i,
                format: 'DMY_STR'
            }
        ];

        let finalDate = null;
        let dateTextCandidate = cleanLines.find(line => /(?:date|dated|dt\.?|on)\b/i.test(line) &&
        ...
        if (cMatch) {
            ...
            if (sMatch) supplierSelect.value = sMatch;
        }

        let typeSelect = document.querySelector('select[name="payment_type"]');
        typeSelect.value = (text.toLowerCase().includes('refund') || text.toLowerCase().includes('return')) ?
            'return' : 'payment';
        ...
        if (typeof $ !== 'undefined' && $(noteElem).summernote) {
            $(noteElem).summernote('code', noteText.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g,
                '&gt;').replace(/\n/g, '<br/>'));
        }

        statusDiv.innerHTML =
            '<span style="color:#28a745"><i class="fas fa-check-circle"></i> Receipt processed! Please verify the values.</span>';

    } catch (err) {
        console.error(err);
        statusDiv.innerHTML =
            `<span style="color:#dc3545"><i class="fas fa-times-circle"></i> ${err?.message || 'Error processing receipt.'}</span>`;
        input.value = '';
    }
}
